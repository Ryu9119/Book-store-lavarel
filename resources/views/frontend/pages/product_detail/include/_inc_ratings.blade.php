<div class="reviews content_text">
    <h4 class="reviews-title"><b>{{ $product->pro_review_total }}</b> product rating <b>{{ $product->pro_name }}</b></h4>
        ...
        <div class="item dashboards_item" style="">
            @foreach($ratingDefault as $key => $item)
                <div class="item_review">
                    ...
                    <span class="item_review-count"><b>{{ $item['count_number']  }}</b> rating</span>
                </div>
            @endforeach
        </div>
        <div class="item dashboards_btn">
            <a href="javascript:;void(0)" title="Submit a rating"
               class="btn btn-success {{ \Auth::id() ? 'js-review' : 'js-show-login' }}">Send rating</a>
        </div>
    </div>
        ...
        <form action="{{ route('ajax_post.user.rating.add') }}" id="form-review" method="POST">
            ...
            <div>
                <p style="margin-bottom: 0">
                    ...
                    <span class="reviews-text" id="reviews-text">Wonderful</span>
                </p>
                <span style="color: red;margin-bottom: 10px;display: block">(Click on the star to rate)</span>
            </div>
            <div>
                <textarea name="content_review"  id="rv_content" cols="30" rows="5" placeholder="">Enter a product review (Minimum 40 characters)</textarea>
                <input type="hidden" name="review" id="review_value" value="5">
                <input type="hidden" name="product_id"  value="{{ $product->id }}">
            </div>
            <button type="submit" style="font-size: 14px;margin-top: 10px" class="btn btn-success js-process-review">Send rating</button>
        </form>
    </div>
