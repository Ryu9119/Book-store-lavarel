...
class ShoppingCartController extends Controller
{
    public function index()
    {
        ...
        $userId = \Auth::user()->id;
        $purchaseAmount = Transaction::where(['tst_user_id' => $userId, 'tst_status' => Transaction::STATUS_SUCCESS])->sum('tst_total_money');

        $discount = 0;
        if ($purchaseAmount >= 50000000 && $purchaseAmount <= 10000000) {
            $discount = 5;
        } else if ($purchaseAmount > 10000000 && $purchaseAmount <= 20000000) {
            $discount = 10;
        } else if ($purchaseAmount > 50000000) {
            $discount = 15;
        }
        \Cart::setGlobalDiscount($discount);
        $viewData = [
            'title_page' => 'Cart List',
            'shopping'   => $shopping,
    ...
    public function postPay(Request $request)
    {
        ...
        if (!\Auth::user()->id) {
            ...
            \Session::flash('toastr', [
                'type'    => 'error',
                'message' => 'Login to perform this feature'
            ]);
        ...
        $data = $request->except("_token", 'discount', 'payment', 'pay');
    ...
    public function update(Request $request, $id)
    {
        if ($request->ajax()) {
            ...
            if (!$product) return response(['messages' => 'There are no products that need updating']);
...
//                    'totalMoney' => \Cart::subtotal(0),
//                    'type'       => 'errors',
//                    'message'    => 'The number of discount codes has expired'
//                ]);
//            }
//
    ...
    public function vnpayReturn(Request $request) {
        ...
        if (session()->has('info_custormer') && $request->vnp_ResponseCode == '00') {
            ...
            try {
                ...
                \Session::flash('toastr', [
                    'type'    => 'error',
                    'message' => 'There was an error that could not pay for the order'
                ]);
                \DB::rollBack();
                return redirect()->to('/');
            }
